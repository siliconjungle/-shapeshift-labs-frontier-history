...
Serializable temporal explanation and causality records for Frontier apps.

`frontier-history` turns event-log entries, CRDT versions, traces, actions, workflow steps, policy decisions, effects, agent/user activity, tests, and replays into one queryable explanation layer.
...
```sh
npm install @shapeshift-labs/frontier-history
```
...
import {
  ...
  explainFieldChange,
  planHistoryUndo
} from '@shapeshift-labs/frontier-history';

const history = compileHistoryTimeline(createHistoryTimeline({
  id: 'app.history',
  records: [{
...
const why = explainFieldChange(history, {
  path: '/entities/todos/t1/done',
  at: 10,
  includeRelated: true
});

const undo = planHistoryUndo(history, {
  scope: 'path',
...
- `createHistoryTimeline`, `defineHistoryRecord`, `compileHistoryTimeline`, `validateHistoryTimeline`, and `queryHistoryTimeline` normalize temporal evidence records.
...
The package benchmark writes `benchmarks/results/frontier-history-package-bench-latest.json` when run from the monorepo. The competitor/control benchmark writes `benchmarks/results/frontier-history-competitors-latest.json`. Frontier-only package measurements cover timeline creation, compilation, validation, path/action queries, explanation, undo planning, windows, registry/provenance graphs, JSONL, and proof hashing.
